...
from paddle_rec import TextRecognizer
...
from cam_ctrl import zoom_set, focus_set
import socket
...
def compare_list_elements(list_1, list_Max, list_Min):  #判断list中相同位置数字大小
    list_1 = [re.sub("[^0-9,.]", '', s) for s in list_1]
    ...
    try:
        for i, (l1, lmx ,lmn) in enumerate(zip(list_1, list_Max, list_Min)):
            if l1 == '' or l1 == '-' or lmx == '-' or lmn == '-':
                continue
            elif (float(l1) > lmx) or (float(l1) < lmn):
                alarm_list.append(i)
                safe = False
...
class Stats(QWidget, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.rec_paddle = TextRecognizer(model_dir='num_tube_rec')
        ...
        # 摄像头
        self.win_streams = [0,
                            'rtsp://admin:34567@192.168.1.10:554/mpeg4/ch1/main/av_stream',
    ...
    # 按钮绑定对应函数
        ...
        self.zoom_in.clicked.connect(self.in_zoom)
        self.zoom_out.clicked.connect(self.out_zoom)
        self.focus_in.clicked.connect(self.in_focus)
        self.focus_out.clicked.connect(self.out_focus)
    ...
    def open_camera(self):
        try:  
            # 调用读取URL的函数;'\
            self.win_stream = self.win_choice.currentText()
            if len(self.win_stream) <= 2:
                self.win_stream = int(self.win_stream)
            ...
            print('s:', self.win_choice.currentText())
    ...
    def save_open(self):
        if self.save_start.text() == '开始保存':
            ...
            time = time.toString("yyyy-MM-dd hh时mm分ss秒")
            time=str(time)
            self.save_time = time
    ...
    def Display(self):
        ...
        while self.cap.isOpened():
            ...
            if self.DisplayLabel.key == 9: # 区域建议
                ...
                box_list0, score = self.word_sear.predict(proposal_area)
            ...
            if self.DisplayLabel.key == 11:
                box_list1, score = self.word_sear.predict(frame)
                ...
                if self.text_rec and self.rec0: # 识别模式
                    ...
                    if not self.DisplayLabel.rec_box:
                        ...
                        for i, box in enumerate(self.DisplayLabel.rec_box):
                            if len(box) == 4:
                                ocr.append(frame[box[1]:box[3],
                                                box[0]:box[2]])
                            elif len(box) == 8:
                                pts = point_sort(box)
                                ocr.append(get_rotate_crop_image(frame, pts))

                        out, elapse = self.rec_paddle(ocr)
